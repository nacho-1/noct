...
```shell
cargo build --package noct --release \
  --target=${ARCH}-unknown-linux-musl \
  --config=target.${ARCH}-unknown-linux-musl.linker=\"rust-lld\"
```
The cross-compiled program `target/${ARCH}-unknown-linux-musl/release/noct` can be
copied to a Linux server or VM and run there.

## License

With the exception of eBPF code, this project is distributed under the terms
